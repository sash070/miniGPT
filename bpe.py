...
encode_rules = []
decode_dict = {i : (i,) for i in range(256)}

def merge(byte_chunks, merge_id, encode_rules, decode_dict):
...
class BPETokenizer():
    ...
    def train(self, text):
        ...
        for i in range(self.vocab_size-256):
            merge_id = 256+i
            merged = merge(byte_chunks, merge_id, self.encode_rules, self.decode_dict)
            if merged == False:
                break
